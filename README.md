...
* **Проверка чертежа** (`drawing_check/`): загрузка чертежа → OCR-ансамбль →
  сверка с требованиями (LLM/эвристики) → замечания; UI на 8501, API на 8502,
  страница логов с кнопкой очистки, диагностика портов
  (внутренний `backend:8502` vs внешний `localhost:8502`).
...
* [docs/drawing_check.md](docs/drawing_check.md) — сервис «Проверка чертежа»
  (порты 8501/8502, OCR, логи)
