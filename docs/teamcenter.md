...
* Версия TC: 11.0
* URL: `http://org-tc2:8080/tc/services/` (SOAP 1.1)
...
1. **Имена операций/сервисов** — константы `SVC_*`, `OP_*` в `mapping.py`.
2. **Пути к элементам ответа** — константы `*_PATH` (кортежи localname'ов от
   SOAP Body, namespace игнорируется). Пример: `("getRequirementsResponse", "requirement")`.
...
После правки запустите `python -m pytest tests/test_stub_soap.py tests/test_sync.py`:
заглушка и клиент используют одни константы, тесты покажут расхождения контракта.
